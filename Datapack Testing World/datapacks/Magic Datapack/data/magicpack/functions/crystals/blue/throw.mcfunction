...
summon trident ~ ~1.4 ~ {pickup:0b,Tags:["projectile","tridentthrown"]}

# apply motion to projectile
execute store result entity @e[type=trident,tag=projectile,limit=1] Motion[0] double 0.001 run scoreboard players get #targetX pos
execute store result entity @e[type=trident,tag=projectile,limit=1] Motion[1] double 0.001 run scoreboard players get #targetY pos
execute store result entity @e[type=trident,tag=projectile,limit=1] Motion[2] double 0.001 run scoreboard players get #targetZ pos

# clean up, ready for the next player
tag @e[tag=projectile] remove projectile
kill @e[tag=direction]

execute as @a at @s run schedule function magicpack:crystals/blue/delete 5s